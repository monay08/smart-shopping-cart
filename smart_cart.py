...
class SmartCartApp:
    def __init__(self, root):
        ...
        self.root.geometry("480x310")  # Set window size to 320x480 for 3.5-inch display
        self.root.resizable(False, False)  # Disable resizing to maintain layout

        self.cart = {}
        self.total = 0.0

        # Removed the header_label entirely

        # Frame for Products List (Adjusted padding)
        self.frame = tk.Frame(root, bg="#f8f9fa", bd=2, relief="ridge")
        self.frame.pack(pady=3, padx=3, fill=tk.BOTH, expand=True)

        # Treeview (Kept the same, but now has more vertical space)
        ...
        self.entry = ttk.Entry(root, font=("Arial", 1))
