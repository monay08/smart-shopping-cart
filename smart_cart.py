...
class SmartCartApp:
    def __init__(self, root):
        ...
        self.root.geometry("320x480")  # Set window size to 320x480 for 3.5-inch display
        ...
        # Frame for Products List (Smaller padding and border)
        self.frame = tk.Frame(root, bg="#f8f9fa", bd=2, relief="ridge")
        self.frame.pack(pady=5, padx=5, fill=tk.BOTH, expand=True)

        # Treeview (Smaller font, reduced height and column widths)
        self.tree = ttk.Treeview(self.frame, columns=("Product", "Quantity", "Price"), show="headings", height=10)
        ...
        # Button Frame (Smaller buttons and padding)
        self.button_frame = tk.Frame(root, bg="#f8f9fa")
        self.button_frame.pack(fill=tk.X, padx=5, pady=5)

        # Remove Item Button
        self.remove_btn = tk.Button(self.button_frame, text="Remove Item", command=self.remove_item, bg="#F9D3D3", fg="black", font=("Arial", 8, "bold"), padx=5, pady=2, relief="flat", bd=0, activebackground="#e74c3c", activeforeground="white")
        self.remove_btn.pack(side=tk.LEFT, ipadx=5, ipady=2, padx=2, pady=2)
        ...
        # Remove One Button
        self.remove_one_btn = tk.Button(self.button_frame, text="Remove One", command=self.remove_one, bg="#FFF3D3", fg="black", font=("Arial", 8, "bold"), padx=5, pady=2, relief="flat", bd=0, activebackground="#ff9900", activeforeground="white")
        self.remove_one_btn.pack(side=tk.LEFT, ipadx=5, ipady=2, padx=2, pady=2)
        ...
        # Clear Cart Button
        self.clear_btn = tk.Button(self.button_frame, text="Clear Cart", command=self.clear_cart, bg="#D3E5F9", fg="black", font=("Arial", 8, "bold"), padx=5, pady=2, relief="flat", bd=0, activebackground="#3399ff", activeforeground="white")
        self.clear_btn.pack(side=tk.RIGHT, ipadx=5, ipady=2, padx=2, pady=2)
    ...
    def remove_item(self):
        ...
        if selected_item:
            ...
            for barcode, item in list(self.cart.items()):
                if item['name'] == Ascendingly == item_values[0]:
